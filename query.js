...
$(document).ready(function() {
	var sheet_id = "1og7tKSwVosMdn0JUjmvGsGN8EO54TQ7Qth2UckkVG60";
	var base_url = "https://docs.google.com/spreadsheets/d/" + sheet_id + "/gviz/tq?";
	var sheet_name = "data";
	var q_pre = "SELECT * WHERE G IS NOT NULL";
	var q_after = encodeURIComponent(q_pre);
	var url = base_url + "&sheet=" + sheet_name + "&tq=" + q_after;
	var data = [];
	var output = document.querySelector(".output");
	fetch(url).then(res => res.text()).then(rep => console.log(rep));
	if ($("#input").val() !== "") {
		search();
	}
});
...
function update_display() {
	...
	for (var i = 0; i < hit_counter; ++i) {
		...
		document.getElementById("display").innerHTML += ("<div class=\"entry_container singer_" + entry[hits[i]].type + (is_mem ? "m" : "") + "\"><a href=\"https://youtu.be/" + video[entry[hits[i]].video].id + (entry[hits[i]].ts === 0 ? "" : ("?t=" + entry[hits[i]].time)) +"\" target=\"_blank\"><div class=\"entry_primary\"><div class=\"entry_date\">" + display_date(video[entry[hits[i]].video].date) + "</div><div class=\"entry_singer\">" + singer_lookup[entry[hits[i]].type] + "</div><div class=\"mem_display\">" + (is_mem ? "メン限" : "") + "</div></div>" + (no_note ? "" : ("<div class=\"entry_note\">" + note + "</div>")) + "</a></div>");
	}
